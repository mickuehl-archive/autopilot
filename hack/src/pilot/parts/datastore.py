...
class Tub(object):
    ...
    def __init__(self, path, inputs=None, types=None, user_meta=[]):
        ...
        if exists:
            ...
            self.meta = {'inputs': inputs,
                         'types': types, 'start': self.start_time}
            for kv in user_meta:
                kvs = kv.split(":")
                if len(kvs) == 2:
    ...
    def get_json_record(self, ix):
        ...
        try:
            ...
            raise Exception(
                'bad record: %d. You may want to run `python manage.py check --fix`' % ix)
        except FileNotFoundError:
            raise
        except:
            print("Unexpected error:", sys.exc_info()[0])
    ...
    def gather_records(self):
        def ri(fnm): return int(os.path.basename(
            fnm).split('_')[1].split('.')[0])

        record_paths = glob.glob(os.path.join(self.path, 'record_*.json'))
        if len(self.exclude) > 0:
            record_paths = [f for f in record_paths if ri(
                f) not in self.exclude]
...
class TubHandler():
    ...
    def new_tub_writer(self, inputs, types, user_meta=[]):
        ...
        tw = TubWriter(path=tub_path, inputs=inputs,
                       types=types, user_meta=user_meta)
        return tw
